...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../user-profile-instructor.php?status=success");
        exit();
    } else {
        // Redirect or show an error message
        header("Location: ../user-profile-instructor.php?status=error");
        exit();
    }
} else {
    // Redirect back if the form wasn't submitted
    header("Location: ../user-profile-instructor.php");
    exit();
}
